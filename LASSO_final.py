...
#w_hat = get_lasso_small(img_blur_train_flat,img_pix_reshape,lambda_lasso)

def get_lasso_small(A,d,la):
    max_iter = 10**4
    tol = 10**(-3)
    ...
    for j in range(max_iter):
        z = w - tau*(A.T@(A@w-d))
        w_old = w
        w = np.sign(z) * np.clip(np.abs(z)-tau*la/2, 0, np.inf)
...
# First pixel ##
img_pix_1 = img_original_train_flat[:,0]
lambda_set = np.logspace(-6, 2, num=18)
len_range = len(lambda_set)

t0 = time.time()
lambda_plot_num = 9
img_test_diff_array = np.zeros((len_range))
img_validate_diff_array = np.zeros((len_range))
for pp, each_lambda in enumerate(lambda_set):
    lambda_lasso = lambda_set[pp]
    ...
    for rr in range(0,np.shape(img_original_train_flat)[1]):
        ...
        img_pix_reshape = img_pix.reshape(np.shape(img_pix)[0],1)
        w_hat = get_lasso_small(img_blur_train_flat,img_pix_reshape,lambda_lasso)
    ...
    img_test_diff_array[pp] = img_test_diff
    img_validate_diff_array[pp] = img_validate_diff

    if pp == lambda_plot_num:
        w_keep = w_hat_series
    if pp == lambda_plot_num-1:
...
lambda_set_abridge = lambda_set[5:14]
img_test_diff_array_abridge = img_test_diff_array[5:14]
img_validate_diff_array_abridge = img_validate_diff_array[5:14]
